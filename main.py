 import os
    ...
    # Используйте ваш актуальный Server ID с Pella.app.
    # Например: https://ВАШ_SERVER_ID.pella.app
    WEBHOOK_BASE_URL = "https://slate-greyhedgehog.onpella.app" # Обновите, если ваш Server ID изменился
    ...
    async def webhook_handler():
        ...
        if request.headers.get('content-type') == 'application/json':
            ...
            logger.info(f"Получены данные (JSON): {json_data}")
            update = Update.de_json(json_data, bot_application.bot)
            logger.info(f"Тип обновления: {type(update)}")
            ...
            # Теперь напрямую await, так как Flask-приложение на Pella.app должно поддерживать async-await
    ...
    # --- ВАЖНО: Удален блок if __name__ == '__main__': ---
    # Pella.app (или другой WSGI-сервер) сам запустит ваше Flask-приложение (переменная 'app').
    # Вам не нужно вызывать app.run() или asyncio.run() здесь.
    # Установка вебхука должна быть выполнена один раз вручную или через отдельный скрипт/интерфейс.
